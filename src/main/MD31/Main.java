...
class MdGUI extends JFrame {
    ...
    MdGUI() {
        ...
        run.setFont(new Font("monotype corsiva", Font.BOLD, 20));
        delete.setFont(new Font("monotype corsiva", Font.BOLD, 20));
        clear.setFont(new Font("monotype corsiva", Font.BOLD, 20));
    ...
    private class ActionRun implements ActionListener {
        ...
        public void actionPerformed(ActionEvent actionEvent) {
            ...
            for (int i = 0; i < list1Model.getSize(); i++) {
                ...
                int count = 0;
                boolean found = false;
                for (int j = 0; j < list1Model.get(i).length(); j++) {
                    try {
                        summ += Integer.parseInt(String.valueOf(list1Model.get(i).charAt(j)));
                        count++;
                        found = true;
                ...
                if (found) {
                    double avr = Math.rint((10.0 * summ) / count) / 10.0;
                    list2Model.addElement(String.valueOf(avr));
                } else {
                    list2Model.addElement("false");
                }
