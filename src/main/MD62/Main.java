    ...
    private class ActionRun implements ActionListener {
        ...
        public void actionPerformed(ActionEvent actionEvent) {
            ...
            for (int i = 0; i < list1Model.size(); i++) {
                String[] temp = list1Model.get(i).split("\\s");
                if (temp.length > 2) {
                    StringBuilder strTemp = new StringBuilder();
                    for (int j = 0; j < temp.length; j++) {
                        if (j != 2) {
                            strTemp.append(temp[j]).append(" ");
                        } else {
                            if (temp[j].charAt(temp[j].length() - 1) == 46)
                                strTemp.append(".");
                        }
                    }
                    list2Model.addElement(strTemp.toString());
                } else {
                    list2Model.addElement(list1Model.get(i));
                }
            }
