    ...
    private class ActionRun implements ActionListener {
        ...
        public void actionPerformed(ActionEvent actionEvent) {
            ...
            for (int i = 0; i < list1Model.getSize(); i++) {
                String[] split = list1Model.get(i).split("\\s");
                StringBuilder temp = new StringBuilder(" ");
                for (String aSplit : split) {
                    try {
                        int number = Integer.parseInt(aSplit);
                        if (number < 1 || number > 26)
                            throw new IllegalArgumentException();
                        else {
                            if (temp.charAt(temp.length() - 1) == 32)
                                temp.deleteCharAt(temp.length() - 1).append((char) (96 + number));
                            else
                                temp.append((char) (96 + number));
                        }

                    } catch (IllegalArgumentException e) {
                        temp.append(aSplit).append(" ");
                    }
                }
